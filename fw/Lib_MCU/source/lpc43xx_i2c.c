...
void I2C_Init(LPC_I2Cn_Type *I2Cx, uint32_t clockrate)
{
  ...
  CGU_PERIPHERAL_T periph;
	...
	if(clockrate<1000)	//make sure SCLH,SCLL not exceed its 16bit value
		return;
	tem = CGU_GetPCLKFrequency(periph) / clockrate; /* EA ANDLI: This was CGU_GetPCLKFrequency(CGU_PERIPHERAL_M4CORE)/clockrate, but CGU_PERIPHERAL_M4CORE has nothing to do with this */
