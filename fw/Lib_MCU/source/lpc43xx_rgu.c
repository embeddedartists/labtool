 ...
 * 					//- RGU_SIG_AES		:AES
 * 					- RGU_SIG_FLASHA	:FLASH B reset signal
 * 					- RGU_SIG_EEPROM	:EEPROM reset signal
 * 					- RGU_SIG_GPIO		:GPIO
 * 					- RGU_SIG_FLASHB	:FLASH B reset signal
...
void RGU_SoftReset(RGU_SIG ResetSignal)
{
	if(ResetSignal < 32){
		...
		//LPC_RGU->RESET_CTRL0 = 0;          //EA ANDLI: According to UM10503 v1.4, the bits are auto cleared to 0 after 1 cycle
	}else{
    if (ResetSignal == RGU_SIG_M0APP){
      LPC_RGU->RESET_CTRL1 = 1 << (ResetSignal - 32);
    }else if (LPC_RGU->RESET_ACTIVE_STATUS1 & (1<<(RGU_SIG_M0APP - 32))) {
      // M0APP is not resetted
      LPC_RGU->RESET_CTRL1 = (1 << (ResetSignal - 32));
      //LPC_RGU->RESET_CTRL1 = 0;          //EA ANDLI: According to UM10503 v1.4, the bits are auto cleared to 0 after 1 cycle
    }else{
      // M0APP has reset asserted, important to not write a zero to the control as that would clear the reset
      LPC_RGU->RESET_CTRL1 = (1 << (ResetSignal - 32)) | (1 << (RGU_SIG_M0APP - 32));
    }
	}
}
...
RGU_SRC RGU_GetSource(RGU_SIG ResetSignal)
{
	...
	if(ResetSignal < 16)
		temp = 3 & (LPC_RGU->RESET_STATUS0 >> (ResetSignal        * 2)); //EA ANDLI: Was missing the *2 multiplier
	else if(ResetSignal < 32)
		temp = 3 & (LPC_RGU->RESET_STATUS1 >> ((ResetSignal - 16) * 2)); //EA ANDLI: Was missing the *2 multiplier
	else if(ResetSignal < 48)
		temp = 3 & (LPC_RGU->RESET_STATUS2 >> ((ResetSignal - 32) * 2)); //EA ANDLI: Was missing the *2 multiplier
	else
		temp = 3 & (LPC_RGU->RESET_STATUS3 >> ((ResetSignal - 48) * 2)); //EA ANDLI: Was missing the *2 multiplier
...
{
	if(ResetSignal < 32)
		return (Bool)!(LPC_RGU->RESET_ACTIVE_STATUS0 & (1 << ResetSignal));  //EA ANDLI: Incorrectly used | when it should have been &
	else
		return (Bool)!(LPC_RGU->RESET_ACTIVE_STATUS1 & (1 << (ResetSignal - 32)));  //EA ANDLI: Incorrectly used | when it should have been &
}
