...
uint8_t Endpoint_Write_Stream_LE(const void* const Buffer,
			                                 ...
			                                 uint16_t* const BytesProcessed)
{
#if 0   //EA ANDLI: original code which does not work
	uint16_t i;
  ...
  if (BytesProcessed == NULL)
  {
    for(i=0;i<Length;i++)
    {
      if (usb_data_buffer_IN_index >= USB_DATA_BUFFER_TEM_LENGTH)
      {
        // Buffer is full and as BytesProcessed is null the transfer
        // fails or suceceeds as a single unit.
  ...
  {
    uint16_t toSend = MIN(USB_DATA_BUFFER_TEM_LENGTH-usb_data_buffer_IN_index, Length);
    for(i=0;i<toSend;i++)
    {
      Endpoint_Write_8(((uint8_t*)Buffer)[i]);      
    }
    *BytesProcessed = toSend;
    if (toSend < Length)
    {
      // Maximum number of bytes have been sent, but more is left.
      return ENDPOINT_RWSTREAM_IncompleteTransfer;
    }
  }
